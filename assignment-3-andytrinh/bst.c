...
struct bst_node* _bst_subtree_remove(int val, struct bst_node* n) {
  ...
  if (n == NULL) {
    ...
    /*
     * If n is NULL, that mea we've reached a leaf node without finding
...
int bst_size(struct bst* bst) {
  assert(bst);
  return _bst_tree_size(bst->root);
}

//helper function
int _bst_tree_size(struct bst_node* node) {
  if (node == NULL) {
    return 0;
  }
  else {
    return _bst_tree_size(node->left) + _bst_tree_size(node->right) + 1;
  }
}
...
int bst_height(struct bst* bst) {
  assert(bst);
  return _bst_tree_height(bst->root);
}

//helper function
int _bst_tree_height(struct bst_node* node) {
  if (node == NULL) {
    return -1;
  }
  else {
    int l = _bst_tree_height(node->left);
    int r = _bst_tree_height(node->right);
    if (l > r)
      return l + 1;
    else
      return r + 1;
...
//helper function
bool _bst_tree_path_sum(struct bst_node* node, int sum) {
  if (node == NULL) {
     return (sum == 0);
  }
  else {
    ...
    if (s == 0 && node->left == NULL && node->right == NULL)
      return 1;
    if(node->left) {
      b = b || _bst_tree_path_sum(node->left, s);
    }
    if(node->right) {
      b = b || _bst_tree_path_sum(node->right, s);
    }
    return b;
  }
}
...
int bst_path_sum(int sum, struct bst* bst) {
  assert(bst);
  bool b = _bst_tree_path_sum(bst->root, sum);
  if (b == true) {
    return 1;
  }
  else
...
void inOrder(struct bst_node *root, struct bst_iterator* iter) {
  struct bst_node *current = root;
  iter->s = stack_create();
  iter->s2 = stack_create();
  bool b = 0;
  while (!b)  {
    if(current !=  NULL) {
      ...
      if (!stack_isempty(iter->s)) {
        current = stack_pop(iter->s);
        stack_push(iter->s2, current->val);
...
struct bst_iterator* bst_iterator_create(struct bst* bst) {
  ...
  assert(iter);
  inOrder(bst->root, iter);
  int array[13] = {};
  int i = 0;
  while(!stack_isempty(iter->s2)) {
    array[i] = stack_pop(iter->s2);
    i++;
  }

  for (int i = 0; i < 13; i++) {
    stack_push(iter->s2, array[i]);
  }
  return iter;
}
